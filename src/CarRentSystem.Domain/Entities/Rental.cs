...
public class Rental
{
    ...
    // Compatibility constructor used by some tests and projections.
    public Rental(Guid id, Guid carId, string customerName, int days, decimal totalPrice)
    {
        Id = id == Guid.Empty ? Guid.NewGuid() : id;
        // Create placeholder car with the requested id so joins by Id work in analytics tests.
        Car = new Car(carId, "Unknown", "Unknown", days > 0 ? totalPrice / days : 0);
        Customer = new EconomyCustomer(customerName);
        Days = days;
