...
| Generics, колекції, LINQ, делегати | `JsonDataStore<T>`, `Result<T>`, `List<T>`, LINQ `Where`, `Sum`, `GroupBy`, `OrderByDescending` | Делегати є в `MenuHandler`, але він не головний UI flow | `Dictionary` і `HashSet` описані в performance analysis |
...
| Рефакторинг | Pricing config, date provider, read/write interfaces з попередньої ітерації | - | Усунено дублювання знижки, safe CLI parsing, XML comments |
